...
1. [ ] Let's make sure to check if we have this required `res.locals.newCharacters` data and if not, invoke the next error handler to break out of the middleware chain and return an error. The error handler should be passed the following properties in an object: 1. [ ] `log`: should include the middleware function name where the error occurred as well as a message noting that the required data was not provided. 1. [ ] `message`: should be an object with an `err` that will be sent back to the client:
